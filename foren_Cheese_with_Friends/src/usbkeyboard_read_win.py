...
def main():
    pcap_path = 'write.pcap'
    pcap = dpkt.pcap.Reader(open(pcap_path, 'rb'))

    keystrokes = []
    for timestamp, buffer in pcap:
        if len(buffer) == 35:
            keystrokes.append(":".join([hex(b)[2:].rjust(2, "0") for b in buffer[-8:]]))
    ...
    execute_keystrokes(keystrokes, KEYMAP_INTL, 0.01, True)

def execute_keystrokes(
        keystrokes: list[str],
        keymap: dict[int, str | Key | KeyCode],
        delay_seconds: float = 0,
        init_numlock: bool = False,
    ) -> None:

    # Set a stop button on F9, and give time to move the cursor on empty document
    time.sleep(2)
    def on_press(key):
        if key == Key.f9:
    ...
    for line in keystrokes:
        if stripped := line.strip():
            ...
            # Capture all keys pressed in a moment
            pressed_keys = set()
            ...
            for key in stroke[2:]:
                if key == 0:
                    break
                pressed_keys.add(keymap[key])

            # Get new pressed and released keys
            ...
            # Skip conditions
            if Key.cmd in pressed_keys: continue

            for key in new_pressed_keys:

                # Skip conditions
                if key == Key.f4 and (keyboard.ctrl_pressed or keyboard.alt_pressed): continue
                if key == Key.tab and (keyboard.ctrl_pressed or keyboard.alt_pressed or keyboard.alt_gr_pressed): continue
                if key == Key.esc and (keyboard.ctrl_pressed or keyboard.alt_pressed or keyboard.alt_gr_pressed): continue
