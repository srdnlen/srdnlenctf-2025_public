...
u16 color, level;        // color of BG, current level & number of lives
...
void die(void)
{
    ...
    // writestring(ST_BLANK, blockmap, 0x136, 0x426);
...
void run_frame(void)
{
    ...
    if (pad0 & KEY_A) {
        if (pad0 & KEY_RIGHT) {
            px += 6;
            if (prev_pad0 != pad0 && global_index < sizeof(global_buffer) - 1) {
                global_index++;
            }
        }
        if (pad0 & KEY_LEFT) {
            px -= 6;
            if (prev_pad0 != pad0 && global_index > 0) {
                global_index--;
            }
        }
        if (pad0 & KEY_UP) {
            if (prev_pad0 != pad0) {
                global_buffer[global_index] += 0x10;
            }
        }
        if (pad0 & KEY_DOWN) {
            if (prev_pad0 != pad0) {
    ...
    if (pad0 & KEY_R) {
        memcpy(local_buffer, global_buffer, sizeof(global_buffer));
        return;
    }
    prev_pad0 = pad0;
...
void print_flag(void) {
    writestring("srdnlen{__REDACTED__FLAG__}", blockmap, 0x242, 0x3F6);
    WaitForVBlank();
    dmaCopyVram((u8 *)blockmap, 0x0000, 0x800);
    while (true) {
        WaitForVBlank();
...
int main(void)
{
    // Compile the print_flag function
    if (FALSE) {
        print_flag();
    }
